...
tf.app.flags.DEFINE_string('labels_file',
                           '/home/sensetime/plz_workspace/Misc/WebVisionStat/webvision/info/info/label_file_all.txt',
                           'Labels file')
...
tf.app.flags.DEFINE_string('dataset_name', 'flickr', '')
tf.app.flags.DEFINE_string('subset_name', 'train', '')
    ...
    if model_checkpoint_path:
        ...
        for var in var_dict:
            ...
            if reader.has_tensor(var_check_name):
                ...
                #print("has variable {} in checkpoint".format(var_check_name))
            elif reader.has_tensor(var):
                available_vars[var] = var_dict[var]
                #print("has variable {} in checkpoint".format(var))
            else:
                print("Variable {} is missing in checkpoint".format(var_check_name))
...
def _find_image_files(data_dir, labels_file):
    ...
    for idx, row in enumerate(webvision_train_file):
        ...
        if relative_path.startswith(FLAGS.dataset_name):
            ...
            index_r = '%.4d' % (FLAGS.index_range)
            if relative_path[8:12] in index_r:
...
def pipeline():
    ...
    with tf.Graph().as_default():
        image_raw_data_dir = FLAGS.train_directory
        num_classes = 1000
        ...
        with tf.Session(config=config) as sess:
            ...
            output_file = os.path.join(FLAGS.output_directory, FLAGS.subset_name + '_' + FLAGS.dataset_name + str(FLAGS.index_range))
